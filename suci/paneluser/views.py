...
from index.decorators import allowed_users, no_autenticado
...
def update_usuarios(request, id):
    queryset = UserEntity.objects.get(id=id)
...
def del_usuarios(request, id):
    if request.method == "POST":
        users = UserEntity.objects.get(id=id)
        users.delete()
...
def usuarios_consultar(request, accion):
    if accion == "consultar":
        if "username" in request.GET:
            ...
            usersl = UserEntity.objects.filter(username=username)
...
def departamentos(request):
    ...
    try:
        ...
        pagina_actual = paginator.page(paginator.num_pages)
    context = {
        "formdp": DepartamentosEForm(),
        "form6": DepartamentosForm(),
        "departamentosp": pagina_actual,
    }
    return render(request, "departamentos.html", context)
